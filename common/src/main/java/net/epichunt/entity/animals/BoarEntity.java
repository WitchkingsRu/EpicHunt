...
public class BoarEntity extends Animal implements NeutralMob{
    private static final EntityDataAccessor<Boolean> ATTACKING = SynchedEntityData.defineId(
            BoarEntity.class, EntityDataSerializers.BOOLEAN);
    ...
    public static final Supplier<EntityType<BoarEntity>> BOAR = Suppliers.memoize(() -> EntityType.Builder.of(BoarEntity::new, MobCategory.CREATURE)
            .sized(1.5f, 1.5f).build("boar"));
...
//            this.mob.swing(InteractionHand.MAIN_HAND);
    ...
    private class WisentAttackPlayersGoal extends NearestAttackableTargetGoal<Player> {
        public WisentAttackPlayersGoal() {
            super(BoarEntity.this, Player.class, 10, true, true, (Predicate)null);
        }

        public boolean canUse() {
            if (BoarEntity.this.isBaby()) {
                return false;
            } else {
                if (super.canUse()) {
                    List<BoarEntity> list = BoarEntity.this.level().getEntitiesOfClass(BoarEntity.class, BoarEntity.this.getBoundingBox().inflate(8.0, 4.0, 8.0));
                    Iterator var2 = list.iterator();

                    while(var2.hasNext()) {
                        BoarEntity goose = (BoarEntity)var2.next();
                        if (goose.isBaby()) {
                            return true;
                        }
                    }
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob ageableMob) {
        return BOAR.get().create(serverLevel);
    }
